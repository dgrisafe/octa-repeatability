...
data_sociodem <- read_xlsx("R&R_PoHX.xlsx", sheet = "PoHx") %>% form_sociodem_vars()

# OCTA data from group's algorithm
data_octa_filename <- "all_quantification_11_26_19.xlsx"
data_octa_global <- read_xlsx(data_octa_filename, sheet = "Global") %>% form_vars()
data_octa_annulus <- read_xlsx(data_octa_filename, sheet = "Annulus") %>% form_vars()
data_octa_hemifield <- read_xlsx(data_octa_filename, sheet = "Hemifield") %>% form_vars()
data_octa_quadrant <- read_xlsx(data_octa_filename, sheet = "Quadrants") %>% form_vars()
data_octa_sector <- read_xlsx(data_octa_filename, sheet = "Sector") %>% form_vars()
data_octa_clockhour <- read_xlsx(data_octa_filename, sheet = "Clockhours") %>% form_vars()

# OCTA data from commercial algorithm
data_octa_commercial <- read_xlsx(data_octa_filename, sheet = "Commercial measurement") %>% form_vars() 
```
...
data_octa_global_inter <- data_octa_global_rename %>% 
  get_1eye() %>% 
  get_inter(window_days = Inf)

# create inter dataset for global data, 365 day window
data_octa_global_inter365 <- data_octa_global_rename %>% 
  get_1eye() %>% 
  get_inter(window_days = 365.25)

# create inter dataset for global data, 180 day window
data_octa_global_inter180 <- data_octa_global_rename %>% 
  get_1eye() %>% 
  get_inter(window_days = 180)
```
...
bind_cols(
  ...
  add_header_above(c(" " = 1, "None" = 1, "Glaucoma" = 1, "None" = 2, "Glaucoma" = 2, "None" = 2, "Glaucoma" = 2, "None" = 2, "Glaucoma" = 2, "None" = 2, "Glaucoma" = 2)) %>% 
