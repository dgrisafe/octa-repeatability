...
form_sociodem_vars <- function(df){
  ...
  df %>% 
    ...
    mutate(
      ...
      surg_proc_catextract_od = base1(`Osx(CE)_OD`) %>% form_surg(),
      surg_proc_catextract_os = base1(`Osx(CE)_OS`) %>% form_surg(),
...
##Table 2: Intra-Visit Repeatability
...
get_repeatability(df = data_octa_global_intra, octa_var = quo(v_diameter)) %>%
  rbind(
    ...
    get_repeatability(df = data_octa_global_intra, octa_var = quo(zeiss_v_area_density)),
    ...
    get_repeatability(df = data_octa_global_intra, octa_var = quo(zeiss_flux))
  ) %>%
  # make html "kable" and round digits
  kable(digits = 3, col.names = c("OCTA Variable", "µ", "Sw", "CRw", "CVw")) %>%
  kable_styling(
    #bootstrap table classes
    bootstrap_options = c( "hover")
  )
