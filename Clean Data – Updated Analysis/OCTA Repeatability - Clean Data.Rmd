---
title: "OCTA Repeatability & Reliability — Clean Data"
...
data_octa_all <- read_xlsx("all_quantification_10_24_19_DG.xlsx", sheet = "names_data")
data_octa_hemifield <- read_xlsx("all_quantification_10_24_19_DG.xlsx", sheet = "Hemifield")
data_octa_quadrant <- read_xlsx("all_quantification_10_24_19_DG.xlsx", sheet = "Quadrants")
data_octa_sector <- read_xlsx("all_quantification_10_24_19_DG.xlsx", sheet = "Sector")
data_octa_clockhour <- read_xlsx("all_quantification_10_24_19_DG.xlsx", sheet = "Clockhours")
data_octa_annulus <- read_xlsx("all_quantification_10_24_19_DG.xlsx", sheet = "Annulus")

# data_octa_all %>% glimpse
```

##Data Dictionary

```{r}
data_dict <- data.frame(stringsAsFactors = F,
  
  variable = c("ptid", "eye_imaged", "time_stamp",
    "name_last", "name_first", "female", "image_area", "image_method", "image_size",
    "v_diameter", "v_area_density", "v_skeleton_density", "v_perimeter_index", "v_complexity_index", "flow_impair_zone", "flux", "msrid"
    ),
  
  label = c("Patient Identification", "Eye Imaged", "Exam Date and Time",
    "Last Name", "First Name", "Female Sex", "Image Area", "Image Method", "Image Size",
    "Vessel Diameter (units?)", "Vessel Area Density (units?)", "Vessel Skeleton Density (units?)", "Vessel Perimeter Index (units?)", "Vessel Complexity Index (units?)", "Flow Impairment Zone (units?)", "Flux (units?)", "Measurement ID"
    )
...
# function that pulls label for given variable
get_label <- function(var){data_dict$label[data_dict$variable==var]}
# get_label("ptid")
```
...
```{r}
data_octa_all_rename <- data_octa_all %>% 
 
  # organize information into separate variables
  separate(col = "...1", sep = "_", remove = TRUE,
           into = c(
             "name_last",
             "name_first",
             "ptid",
  ...
  rename(
    v_diameter = "Vessel Diameter",
    v_area_density = "Vessel Area Density",
    v_skeleton_density = "Vessel Skeleton Density",
    v_perimeter_index = "Vessel Perimeter Index",
    v_complexity_index = "Vessel Complexity Index",
    flow_impair_zone = "Flow Impiarment Zone",
    flux = "Flux"
  ) 

# data_octa_all_rename %>% glimpse
```
...
```{r}
data_octa_all_format <- data_octa_all_rename %>% 
  mutate(
    ...
    eye_imaged = case_when(
      eye_imaged == "OS" ~ 1,
      eye_imaged == "OD" ~ 2
    ) %>% factor(levels = c(1, 2), labels = c("OS", "OD")),
  ...
  select(msrid, ptid, eye_imaged, time_stamp, exam_date, everything())
  
  

# data_octa_all_format %>% glimpse
```
...
# function that gets the label from the data dictionary
get_dict_label <- function(n){data_dict[["label"]][[n]]}
    
# assign label to each variable
label(data_octa_all_format[["ptid"]])               <- get_dict_label(1)
label(data_octa_all_format[["eye_imaged"]])         <- get_dict_label(2)
label(data_octa_all_format[["time_stamp"]])         <- get_dict_label(3)
# label(data_octa_all_format[["name_last"]])          <- get_dict_label(4)
# label(data_octa_all_format[["name_first"]])         <- get_dict_label(5)
label(data_octa_all_format[["female"]])             <- get_dict_label(6)
label(data_octa_all_format[["image_area"]])         <- get_dict_label(7)
label(data_octa_all_format[["image_method"]])       <- get_dict_label(8)
label(data_octa_all_format[["image_size"]])         <- get_dict_label(9)
label(data_octa_all_format[["v_diameter"]])         <- get_dict_label(10)
label(data_octa_all_format[["v_area_density"]])     <- get_dict_label(11)
label(data_octa_all_format[["v_skeleton_density"]]) <- get_dict_label(12)
label(data_octa_all_format[["v_perimeter_index"]])  <- get_dict_label(13)
label(data_octa_all_format[["v_complexity_index"]]) <- get_dict_label(14)
label(data_octa_all_format[["flow_impair_zone"]])   <- get_dict_label(15)
label(data_octa_all_format[["flux"]])               <- get_dict_label(16)
label(data_octa_all_format[["msrid"]])              <- get_dict_label(17)
...
data_octa_all_intra <- data_octa_all_format %>% 
...
Inter-visit data was restricted to the last measurement of the day for each eye. If a patient had their right eye measured:    
...
- The third measurement from Monday,  

- The fifth measurement from Wednesday, and  

- The second measurement from Friday.
...
data_octa_all_inter <- data_octa_all_format %>% 
...
data.frame(
  ...
  n_participants = c(
    data_octa_all_intra %>% group_by(ptid) %>% summarise(n()) %>% nrow(),
    data_octa_all_inter %>% group_by(ptid) %>% summarise(n()) %>% nrow()
    ),
  
  #number of eyes
  n_eyes = c(
    data_octa_all_intra %>% group_by(ptid, eye_imaged) %>% summarise(n()) %>% nrow(),
    data_octa_all_inter %>% group_by(ptid, eye_imaged) %>% summarise(n()) %>% nrow()
    ),
  
  # number of measurements
  n_measurements = c(
    data_octa_all_intra %>% nrow(),
    data_octa_all_inter %>% nrow()
    )
...
# data_octa_all_format_1eye <- data_octa_all_format %>%
...
print_table1 <- function(df){
  ...
  # "msrid", "ptid", "name_last", "name_first", "time_stamp", "exam_date"
...
```{r}
print_table1(data_octa_all_intra)
```
...
```{r}
print_table1(data_octa_all_inter)
```
...
scat_octa_sid <- function(octa_var){
  ...
  set.seed(1998)
  ptid_sample10 <- data_octa_all_format %>% dplyr::filter(eye_imaged == "OD") %>% select(ptid) %>% sample_n(10) %>% unlist() %>% as.character()
  
grid.arrange(ncol = 2, left = get_label(octa_var), top = "OCTA Measures by Patient", bottom = get_label("ptid"),

             data_octa_all_intra %>%
               dplyr::filter((ptid %in% ptid_sample10) & (eye_imaged == "OD")) %>%
               ggplot(aes_string(x = "ptid", y = octa_var, group = "ptid")) +
               geom_point(alpha = 0.1, size = 5, color = "red") +
               ylim(min(data_octa_all_format[[octa_var]]), max(data_octa_all_format[[octa_var]])) +
               ggtitle("Intra") +
               ylab("") +
               xlab("") +
               theme(
...
grid.arrange(ncol = 2, left = get_label(octa_var), top = "OCTA Measures Over Time", bottom = get_label("time_stamp"),
...
get_blandaltman <- function(df, octa_var){
  ...
  octa_name <- octa_var %>% as_label() %>% get_label()

  # intra-visit dataset
  data_blandalt <- df %>%

    # select only the measurement ID and the OCTA variable of interest
    select(ptid, eye_imaged, order_obs, msrid, !!octa_var) %>%
...
# get_blandaltman(df = data_octa_all_intra, octa_var = quo(v_area_density))
```
...
###Vessel Diameter
```{r, warning = FALSE}
get_blandaltman(df = data_octa_all_intra, octa_var = quo(v_diameter))
```

###Vessel Skeleton Density
```{r, warning = FALSE}
get_blandaltman(df = data_octa_all_intra, octa_var = quo(v_skeleton_density))
```

###Vessel Perimeter Index
```{r, warning = FALSE}
get_blandaltman(df = data_octa_all_intra, octa_var = quo(v_perimeter_index))
```

###Vessel Complexity Index
```{r, warning = FALSE}
get_blandaltman(df = data_octa_all_intra, octa_var = quo(v_complexity_index))
```

###Flow Impairment Zone
```{r, warning = FALSE}
get_blandaltman(df = data_octa_all_intra, octa_var = quo(flow_impair_zone))
```

###Flux
```{r, warning = FALSE}
get_blandaltman(df = data_octa_all_intra, octa_var = quo(flux))
```
...
```{r}
data_octa_all_intra %>% 
  filter(msrid %in% c(50, 51, 55, 56, 121, 122, 139, 140, 175, 176, 592, 593)) %>% 
  kable(digits = 3) %>%
  kable_styling(
    #bootstrap table classes
...
###Vessel Area Density
```{r, warning = FALSE}
get_blandaltman(df = data_octa_all_inter, octa_var = quo(v_area_density))
```

###Vessel Diameter
```{r, warning = FALSE}
get_blandaltman(df = data_octa_all_inter, octa_var = quo(v_diameter))
```

###Vessel Skeleton Density
```{r, warning = FALSE}
get_blandaltman(df = data_octa_all_inter, octa_var = quo(v_skeleton_density))
```

###Vessel Perimeter Index
```{r, warning = FALSE}
get_blandaltman(df = data_octa_all_inter, octa_var = quo(v_perimeter_index))
```

###Vessel Complexity Index
```{r, warning = FALSE}
get_blandaltman(df = data_octa_all_inter, octa_var = quo(v_complexity_index))
```

###Flow Impairment Zone
```{r, warning = FALSE}
get_blandaltman(df = data_octa_all_inter, octa_var = quo(flow_impair_zone))
```

###Flux
```{r, warning = FALSE}
get_blandaltman(df = data_octa_all_inter, octa_var = quo(flux))
```
...
```{r}
data_octa_all_inter %>% 
  filter(msrid %in% c(30, 32, 42, 45, 73, 75, 138, 141, 187, 188, 271, 272, 294, 295, 330, 331, 538, 535)) %>% 
  kable(digits = 3) %>%
  kable_styling(
    #bootstrap table classes
    bootstrap_options = c( "hover")
...
get_repeatability <- function(df, octa_var){
  ...
  df %>%
    ...
    # select only the measurement ID and the OCTA variable of interest
    select(ptid, eye_imaged, order_obs, !!octa_var) %>%
...
get_repeatability(df = data_octa_all_intra, octa_var = quo(v_diameter)) %>%
  rbind(
    get_repeatability(df = data_octa_all_intra, octa_var = quo(v_area_density)),
    get_repeatability(df = data_octa_all_intra, octa_var = quo(v_skeleton_density)),
    get_repeatability(df = data_octa_all_intra, octa_var = quo(v_perimeter_index)),
    get_repeatability(df = data_octa_all_intra, octa_var = quo(v_complexity_index)),
    get_repeatability(df = data_octa_all_intra, octa_var = quo(flow_impair_zone)),
    get_repeatability(df = data_octa_all_intra, octa_var = quo(flux))
  ) %>%
...
```{r, warning = FALSE}
icc_tabulate(data_octa_all_intra)
```

##Inter-Visit ICC

```{r, warning = FALSE}
icc_tabulate(data_octa_all_inter)
```
...
model1.fit <- lmer(v_area_density ~ (1|ptid), data=data_octa_all_intra)
summary(model1.fit)

# model2.fit <- lmer(v_area_density ~ SS + (1|ptid), data=data_octa_all_intra)
# summary(model2.fit)
```
