...
get_ba_data <- function(df, outcome){
  ...
  get_blandaltman_data(df, outcome_dplyr) %>% 
    ...
    select(ptid, eye_imaged, order_obs, eyeid, outcome, diff, mean)
...
figure1_facet_vertical <- ba_data %>% 
  ggplot(aes(x = mean, y = diff)) +
  geom_point(size = 0.1, alpha = 1) +
...
dev.copy(tiff, "figure1_facet_v_paper.tiff", width = 1200, height = 2000, res = 300)
figure1_facet_vertical
dev.off()


# create bland altman plot from data: horizontal for presentations
figure1_facet_horizontal <- ba_data %>% 
  ggplot(aes(x = mean, y = diff)) +
  geom_point(size = 0.1, alpha = 1) +
  geom_hline(data = ba_data_summary, aes(yintercept = diff_mean), colour = "blue", size = 0.5, linetype = 2) +
  geom_hline(data = ba_data_summary, aes(yintercept = diff_l95cl), colour = "red", size = 0.5, linetype = 2) +
...
dev.copy(tiff, "figure1_facet_h_slides.tiff", width = 2000, height = 1200, res = 300)
...
get_big_stats_table_2eyes <- function(df){
  ...
  get_adjusted_icc <- function(measurement, df){
    ...
    get_cl95 <- function(x){
      ...
      boot_x <- bootMer(model_2eye, x, nsim = 100)   # nsim = 1000 is standard, but 1hr. 100 probably sufficient. 10 fast enough to edit code
  ...
  vars_reliable_table <- c("mu", outer(c("icc", "sw", "crw", "cvw"), c("", "_lower_cl", "_upper_cl"), FUN = "paste0")) 
  ...
  table_data <- bind_rows(
    ...
    select(var_name, contains("mu_"), contains("icc_"), contains("sw_"), contains("crw_"), contains("cvw_")) # then order by reliability: mu, icc, sw, etc.
  ...
  table_data_pretty %>% 
    kable(digits = 3, format = "html") %>% 
        ...
        # group column headers
        add_header_above(c(" " = 1, "Normal" = 1, "Glaucoma" = 1, rep(c("Normal" = 2, "Glaucoma" = 2), times = 4))) %>% 
        add_header_above(c("OCTA Variable" = 1, "Mean" = 2, "ICC" = 4, "Sw" = 4, "CRw" = 4, "CVw" = 4))
