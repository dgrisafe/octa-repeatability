...
grid.arrange(ncol = 2, left = var_labels[octa_var], top = "OCTA Measures by Patient", bottom = var_labels["ptid"],
  ...
  get_plot_scatter(df = data_octa_global_intra, title = "Intra", line_color = "red"),
  get_plot_scatter(df = data_octa_global_inter, title = "Inter", line_color = "blue")
...
####Inter-Visit Repeatability Normal{.tabset}
#####Normal, All
```{r}
data_octa_global_inter %>%
  filter(glc_dx_bin == "Normal") %>% 
...
####Inter-Visit Repeatability Glaucoma{.tabset}
...
* Normal (n = `r get_n_disease(data_octa_global_intra, "Normal")`).

* Glaucoma (n = `r get_n_disease(data_octa_global_intra, "Glaucoma")`).

```{r}
get_big_stats_table(df_normal = data_octa_global_intra, df_glaucoma = data_octa_global_intra)
```

###Table 2b: Inter-Visit Repeatability

Data was restricted differently by glaucoma status for inter-visits that occurred on different days. This was to avoid differences in data sparsity. Participants who had repeated visits with a window of:

* 1 year for normal participants without glaucoma (n = `r get_n_disease(data_octa_global_inter365, "Normal")`).

* 6 months for participants with glaucoma (n = `r get_n_disease(data_octa_global_inter180, "Glaucoma")`).

```{r}
get_big_stats_table(df_normal = data_octa_global_inter365, df_glaucoma = data_octa_global_inter180)
```
