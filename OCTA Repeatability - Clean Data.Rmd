---
title: "OCTA Reliability — Clean Data"
...
The following results are for all eyes included. Results for a single eye—chosen at random—from each participant may be shown later. Bruce did explain a way to calculate ICC's for all measurements on both eyes of each participant. This may be coded later.
...
get_flow_chart <- function(df_intra, df_inter){
  ...
  data.frame(
    ...
    n_participants = c(
      df_intra %>% group_by(ptid) %>% summarise(n()) %>% nrow(),
      df_inter %>% group_by(ptid) %>% summarise(n()) %>% nrow()
      ),
    
    #number of eyes
    n_eyes = c(
      df_intra %>% group_by(ptid, eye_imaged) %>% summarise(n()) %>% nrow(),
      df_inter %>% group_by(ptid, eye_imaged) %>% summarise(n()) %>% nrow()
      ),
...
bind_rows(
  get_flow_chart(df_inter =  data_octa_global_inter_1eye, df_intra = data_octa_global_intra_1eye),
  get_flow_chart(df_inter =  data_octa_global_inter_2eye, df_intra = data_octa_global_intra_2eye)
) %>% 
  kable() %>% 
    kable_styling(bootstrap_options = c("striped", "hover")) %>% 
    pack_rows("Global Data - Single Eye, Random", 1, 2) %>% 
    pack_rows("Global Data - Both Eyes", 3, 4) 
...
# single eye
count_1eye <- get_count_obs(data_octa_global_1eye)
count_intra_1eye <- get_count_obs(data_octa_global_intra_1eye)
count_inter_1eye <- get_count_obs(data_octa_global_inter_1eye)

# both eyes
count_2eye <- get_count_obs(data_octa_global_2eye)
count_intra_2eye <- get_count_obs(data_octa_global_intra_2eye)
count_inter_2eye <- get_count_obs(data_octa_global_inter_2eye)
```

###Single Eye, Random

After randomly selecting for one eye per participant,

**Overall** there were:

- `r count_1eye$measurements$normal` measurements of `r count_1eye$patients$normal` patients without glaucoma

- `r count_1eye$measurements$glaucoma` measurements of `r count_1eye$patients$glaucoma` patients with glaucoma

Among **intravisit** data there were:

- `r count_intra_1eye$measurements$normal` measurements of `r count_intra_1eye$patients$normal` patients without glaucoma

- `r count_intra_1eye$measurements$glaucoma` measurements of `r count_intra_1eye$patients$glaucoma` patients with glaucoma

Among **intervisit** data there were:

- `r count_inter_1eye$measurements$normal` measurements of `r count_inter_1eye$patients$normal` patients without glaucoma

- `r count_inter_1eye$measurements$glaucoma` measurements of `r count_inter_1eye$patients$glaucoma` patients with glaucoma


###Both Eyes

When including both eyes of participants,

**Overall** there were:

- `r count_2eye$measurements$normal` measurements of `r count_2eye$patients$normal` patients without glaucoma

- `r count_2eye$measurements$glaucoma` measurements of `r count_2eye$patients$glaucoma` patients with glaucoma

Among **intravisit** data there were:

- `r count_intra_2eye$measurements$normal` measurements of `r count_intra_2eye$patients$normal` patients without glaucoma

- `r count_intra_2eye$measurements$glaucoma` measurements of `r count_intra_2eye$patients$glaucoma` patients with glaucoma

Among **intervisit** data there were:

- `r count_inter_2eye$measurements$normal` measurements of `r count_inter_2eye$patients$normal` patients without glaucoma

- `r count_inter_2eye$measurements$glaucoma` measurements of `r count_inter_2eye$patients$glaucoma` patients with glaucoma
...
# print_table1(data_octa_global_intra_1eye)
```
...
###Measurements, Single Eye
```{r, warning = FALSE}
print_table1_measure(data_octa_global_intra_1eye)
```

###Measurements, Both Eyes
```{r, warning = FALSE}
print_table1_measure(data_octa_global_intra_2eye)
```
...
###Participants
```{r, message = FALSE}
print_table1_people(data_octa_global_inter_1eye)
```

###Measurements, Single Eye
```{r}
print_table1_measure(data_octa_global_inter_1eye)
```

###Measurements, Both Eyes
```{r}
print_table1_measure(data_octa_global_inter_2eye)
```
...
get_repeat_kable <- function(df_kable){
  ...
  df_kable %>% 
    ...
    kable(digits = 3, col.names = c("OCTA Variable", "n_measure", "µ", "Sw", "Sw 95% CL", "CRw", "CRw 95% CL", "CVw", "CVw 95% CL")) %>%
    kable_styling(
      #bootstrap table classes
      bootstrap_options = c( "hover")
    )
...
Restrict to repeat measurements within a given window of time. 12 months for normal, and 6 months for glaucoma, to get similar amounts of data.
...
icc_tabulate <- function(df){
  ...
  # functions for formatting stats and confidence intervasls
...
<span style="color:blue">"I also added a model to test whether SS influences reliability. The reference level is ‘10’ now, which is what we want, and you can see that SS is highly associated with VAD, meaning it affects reliability." - BB 9/4/2019</span>

```{r}
model_ss_fit <- lmer(v_area_density ~ signal_strength + (1|ptid), data = data_octa_global_intra_1eye)
summary(model_ss_fit)
```


#Mixed Models with Both Eyes
...
df_normal   <- data_octa_global_intra_2eye %>% filter(glc_dx_bin == "Normal")
df_glaucoma <- data_octa_global_intra_2eye %>% filter(glc_dx_bin == "Glaucoma")

# function that returns ICC w/95% confidence intervals 
get_adjusted_icc <- function(df, measurement){
  ...
  # calculate ICC confidence intervals with bootstrap methods
  ...
  #     https://stats.stackexchange.com/questions/184767/how-do-i-calculate-the-confidence-interval-of-an-icc
    ...
    boot_icc <- bootMer(model_2eye, calc_icc, nsim = 10)   # nsim = 1000 is standards, but 10 makes it run fast, remember to switch to 1000 when running the final thing
    
    # draw from the bootstrap distribution the usual 95% upper and lower confidence limits
    cl95 <- quantile(boot_icc$t, c(0.025, 0.975))
...
get_icc_disease_group <- function(df, var_names){
  
  # get name of disease group
  disease_group_name <- deparse(substitute(df))
...
table_data <- bind_rows(
  get_icc_disease_group(df_normal,   octa_vars_select_vars),
  get_icc_disease_group(df_glaucoma, octa_vars_select_vars)
) %>% 
  ...
  # spread data
  pivot_wider(id_cols = var_name, values_from = c(icc, icc_lower_cl, icc_upper_cl, pretty_icc, pretty_icc_cl), names_from = disease) %>% 
  # order variables into proper format
  select(var_name, ends_with("normal"), ends_with("glaucoma"))
...
table_data_pretty %>% 
  kable(digits = 3, format = "html") %>% 
      ...
      # group column headers
      add_header_above(c(" " = 1, "Normal" = 2, "Glaucoma" = 2)) %>% 
      add_header_above(c("OCTA Variable" = 1, "ICC" = 4))
```
