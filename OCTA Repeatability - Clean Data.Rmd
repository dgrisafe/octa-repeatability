...
data_octa_global <- read_xlsx(data_octa_filename, sheet = "Annulus") %>% form_vars() %>% exclude_patients()
...
print_table1_people <- function(df){
  ...
  df_table1 <- df_table1 %>% 
    ...
    CreateTableOne(data = df_table1, vars = Vars_table1, strata = "glc_dx_bin") %>%
      ...
      add_footnote(notation = "number", label = "Participants with glaucoma in only one eye may be counted in both 'Normal' and 'Glaucoma' groups. This situation was rare.")
