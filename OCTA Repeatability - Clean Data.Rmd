...
* 1 year for normal participants without glaucoma (n = `r data_octa_global_inter365 %>% filter(glc_dx_bin == "Normal") %>% group_by(ptid) %>% n_groups()`).

* 6 months for participants with glaucoma (n = `r data_octa_global_inter180 %>% filter(glc_dx_bin == "Glaucoma") %>% group_by(ptid) %>% n_groups()`).
